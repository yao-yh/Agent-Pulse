...
- 只支持可以修改 `base_url`、`apiBaseUrl`、`ANTHROPIC_BASE_URL`、`endpoint` 等配置的 API 密钥模式。
- 不支持 OAuth、订阅账号登录、ChatGPT/claude.ai 网页账号通道等不可直接改 base URL 的模式。
- Web 控制台默认修改当前用户级配置。CLI 默认仍只修改当前工作区配置，用户级配置必须显式使用 `--scope user --yes`。
...
工具配置完成后，请求会被路由到以下本地代理入口。`/proxy/` 后面的部分是本地映射标识 `proxyKey`，代理会用它查找真实上游和 API 标准：
...
如果某个 `proxyKey` 没有对应映射，代理会返回 `404 proxy_mapping_not_found`，不会自动转发到官方默认上游。
...
- `提示`：缺失配置、无法解析、配置结构未识别或操作结果。
- `操作`：支持“替换”和“回滚”。

当前阶段，“扫描电脑上的所有 Agent”是指扫描 AgentPulse 已注册适配器支持的工具。新增适配器后会自动进入该列表。
...
- `risks`：风险提示。
- `rollback`：回退相关信息。
- `proxyRoute`：后续代理层可使用的路由配置。

如果某个工具的配置结构无法可靠识别，计划会降级为 `command.suggestion`，不会强行写入配置。
...
代理层会把这份映射加载到内存。后续请求进入 `/proxy/claude-code` 时，会优先按内存映射转发到原始上游，而不是只依赖环境变量。
...
如果用户级配置文件不存在，但适配器可以安全创建配置文件，确认弹窗会显示“将创建配置文件”。此时回滚会删除本次新建的配置文件。
...
2. 写入备份元数据。
3. 修改目标配置文件。
4. 调用对应集成适配器验证配置是否已指向 AgentPulse 代理。
...
注意：`--yes` 只用于允许写入非工作区作用域，不代表跳过备份或验证。
...
回滚成功后，AgentPulse 会删除该 Agent 的代理映射，避免配置已经恢复但代理仍继续使用旧映射。
...
如果请求成功经过代理，应能看到对应模型提供方、请求方法、路径、状态和耗时。

## 9. 常见问题

### plan 只有 command.suggestion，没有 file.patch

说明 AgentPulse 没有可靠识别该工具的可写基础地址字段。此时不会强行修改配置，需要人工检查工具配置结构。

### install 后 verification.ok 为 false

说明配置文件写入后没有验证到预期代理地址。可能原因：

- 工具配置字段名称不符合当前适配器的识别规则。
- 配置文件被其它进程修改。
...
说明该 Agent 没有已应用计划对应的备份。行级回滚不会回滚其他 Agent 的备份，以避免误恢复错误配置。
