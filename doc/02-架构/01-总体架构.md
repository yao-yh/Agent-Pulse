...
1. 核心能力不绑定某个具体 AI 工具。
2. 通过集成适配器适配不同工具。
3. 通过 Hook 和回调采集任务进度、生命周期和本地行为上下文。
4. 通过通知渠道及时通知用户任务进展、失败和等待确认。
5. 通过能力盘点和本地探针直接扫描本地技能、MCP、插件和配置，避免完全依赖 Agent 自报。
6. 通过代理捕获模型请求和响应，作为上下文分析增强能力。
7. 通过会话记录导入补充历史记录。
8. 通过插件扩展通知、分析、策略和工具适配。
9. 默认本地运行、本地存储、本地分析。
...
- `eventType`：标准事件类型。
- `correlationId`：用于关联 Hook 事件、代理请求和会话记录。
- `raw`：原始事件，可能需要脱敏后存储。
- `normalized`：标准化后的结构，供 Web、分析器和策略使用。
...
- 每个适配器自行处理工具的配置文件、环境变量、基础地址和路径规则。
- 适配器只生成计划，不直接静默修改。
- apply 前必须有备份。
...
AgentPulse 需要增加 `packages/inventory`，用于独立扫描本地技能、MCP 服务器、插件和 Agent 配置。

该模块职责：

- 扫描项目级、用户级、全局级 Agent 配置。
- 发现技能、MCP 服务器、插件和工具扩展。
- 读取 `SKILL.md`、MCP 配置、插件 manifest 等元数据。
- 记录来源路径、启用状态、配置摘要、最后修改时间。
- 对比 Agent 自报能力与本地扫描结果。
- 为分析器提供技能与 MCP 使用分析基线。
...
AgentPulse 需要增加 `packages/probes`，用于封装系统判断、文件判断、命令判断、配置解析、技能判断、MCP 判断、插件判断和敏感信息判断等通用能力。

该层职责：

- 判断当前系统、命令行环境、工作区和配置目录。
- 判断文件、目录、权限、大小和修改时间。
- 判断命令是否存在以及版本信息。
...
详细方案见 [本地探针模块详细设计](../03-模块详细设计/03-本地探针/README.md)。
